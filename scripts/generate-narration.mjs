...
for (const ev of events) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) { console.error('→ Key sai hoặc chưa bật quyền Text to Speech.'); break; }
      if (res.status === 429 || /quota|credit|exceeded/i.test(msg)) {
        console.error('→ HẾT CREDIT. Đổi ELEVENLABS_API_KEY sang tài khoản khác rồi chạy lại — file đã tạo sẽ được bỏ qua.');
        break;
      }
